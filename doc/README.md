...
Every write in MongoDB has a timestamp assigned, in particular a logical time based on a variation of the [Lamport Clock](https://en.wikipedia.org/wiki/Lamport_timestamp). A snapshot is a point in time that reflects only the prior writes. This means that if I have the following sequence of writes:
...
Use retryable writes whenever possible in order to avoid a transaction abortion because of transient failures, like network errors.
