...
- **Snapshot**: the transaction will see a point in time snapshot of the data, only if it commits with `{w: majority}` write concern. By using **local** or **majority**, you get consistent snapshot per partition, whereas with **snapshot** read concern all the shards begin the transaction at the same `ClusterTime`. In practice, if I have a distributed transaction involving two shards X and Y, with target snapshot at time 102, X at cluster time 101 and Y at cluster time 105, the snapshot of each shard is not going to see operations executed after 102, independently from the majority commit point. Starting from version 5.0, you can choose an older cluster time instead of using the current one.
